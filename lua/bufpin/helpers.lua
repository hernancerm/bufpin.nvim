...
--- Normalize the buf argument taken by the functions which act on bufs: nothing
--- or 0 means the current buf, a bufnr means that buf and a list means those
--- bufs. Repeated bufs are dropped, keeping the leftmost occurrence.
---@param bufnr (integer|integer[])?
---@return integer[]
function h.to_bufnr_list(bufnr)
  local current_bufnr = vim.fn.bufnr()
  local items = bufnr
  if type(items) ~= "table" then
    items = { bufnr or current_bufnr }
  end
  local bufnrs = {}
  for _, item in ipairs(items) do
    if item == 0 then
    ...
    if h.table_find_index(bufnrs, item) == nil then
      table.insert(bufnrs, item)
    end
  end
  return bufnrs
end
